...
| `CC_LANGFUSE_REDACT_SECRETS` | Redact PEM private keys, labeled credential assignments (`api_key`/`token`/`password`/... = value), and unlabeled high-entropy tokens from every string span attribute before export, replacing each with a `[REDACTED:...]` marker (default `true`). Structural attributes the SDK sets itself (usage/cost details, model name/params, observation type/level, environment/release/version) are left alone. Requires `langfuse>=4.9`; if the installed SDK is older, tracing is disabled rather than sent unmasked. | No |
